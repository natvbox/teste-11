...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    // Para evitar confusão: muitas chamadas de upsert atualizam só lastSignedIn.
    // Nesses casos, o role NÃO deve ser sobrescrito e o log deve refletir o role real no banco.
    const existing = await getUserByOpenId(user.openId);
    const existingRole = (existing as any)?.role as string | undefined;
    ...
    // Recarrega para logar o estado real após o upsert
    const after = await getUserByOpenId(user.openId);
    const afterRole = (after as any)?.role as string | undefined;
    const roleShown = afterRole ?? values.role ?? existingRole ?? "user";
    const roleNote = values.role ? "(definido)" : existingRole ? "(mantido)" : "(padrão)";

    console.log(`[Database] ✅ Usuário ${user.openId} upserted com role: ${roleShown} ${roleNote}`);
  } catch (error) {
