...
// ✅ O backend precisa ler o mesmo cookie que o login escreve.
// - Se SESSION_COOKIE_NAME estiver definido, ele tem prioridade.
// - Caso contrário, usa o COOKIE_NAME padrão do projeto.
const SESSION_COOKIE_NAME =
  (ENV.sessionCookieName && ENV.sessionCookieName.length > 0
...
class SDKServer {
  ...
  private getSessionSecret() {
    ...
    const secret = ENV.cookieSecret || process.env.COOKIE_SECRET || "default-secret-for-development-only";

    // ✅ Segurança: em produção, não permite secret padrão/fraco
    if (isProd && secret === "default-secret-for-development-only") {
      throw new Error(
        "[SECURITY] COOKIE_SECRET (ou COOKIE_SECRET no ENV) é obrigatório em produção. Defina um segredo forte."
      );
    }
